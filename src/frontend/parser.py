...
class _Parser(object):
    ...
    def if_stmt(self, idx):
        ...
        # Try consuming else token, if not return. 
        try:
            _, else_idx = self.consume(block_idx, TOKEN_IDS["ELSE"])
        except SileParseError:
            # No else block return. 
            n = Node("if").addkid(expr).addkid(block)
            return n, block_idx
        ...
        n = Node("if").addkid(expr).addkid(block).addkid(next_node)
        return n, next_idx
    ...
    def break_stmt(self, idx):
        break_token, break_idx = self.consume(idx, TOKEN_IDS["BREAK"])
        try:
    ...
    def continue_stmt(self, idx):
        continue_token, continue_idx = self.consume(idx, TOKEN_IDS["CONTINUE"])
        try:
            name, name_idx = self.consume(continue_idx, TOKEN_IDS["NAME"])
        except SileParseError:
            n = Node("continue")
